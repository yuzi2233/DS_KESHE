...
public class Flow_Pane_every {
    ...
    Picture pic;
    Subway sub;
    Flow_Pane_every(Subway su, Picture pic) {
        this.sub=su;
        int flag=0;
        this.pic=pic;
        ...
        for (int i = 0; i < LINE_NUM; i++) {
            ...
            HBox []rectangle_pane_every=new HBox[row_num]; //每一个面板分配多少行
            for(int n=0;n<row_num;n++)
            {
                rectangle_pane_every[n]=new HBox();
            }
            //rectangle_pane_every.setPadding(new Insets(10,10,10,10));
            int m=0;
            int n=0;
            ...
            for(Object a:sub.text.Return_Line_List()[i])
            {
                ...
                rectangle_pane_every[n].getChildren().add(bu[m]);
            ...
            col_pane.getChildren().addAll(rectangle_pane_every);
