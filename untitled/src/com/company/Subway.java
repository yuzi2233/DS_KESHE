...
public class Subway {
    ...
    void Price(String station,int s,int distance,String []transfer,int []last_tranline,int []last_tran_sta) //s为出发站最近的站
    {
        ...
        for(int i=0;i<this.text.Return_Line_Num();i++)
        {
            ...
            for(Object a:this.text.Return_Line_List()[i])
            {
                ...
                if(s==dis_o)
                {
                    dis= this.GetOneLinePrice((String)a,station,i);
                    num=Return_Price(dis);
                }
                else {
                    num = Return_Price(this.graph.test_matrix[s][dis_o] + distance + dis);
                }
    ...
    private int GetOneLinePrice(String a, String station, int i) {
        ...
        int min;
        boolean flag=false;
        LinkedHashMap line= text.Return_Distance()[i];
        ...
        if(flag==true)
        {
            return  GetOneLinePrice(station,a,i);
        }
        return 0;
    }
